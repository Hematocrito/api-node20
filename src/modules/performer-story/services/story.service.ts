...
export class StoryService {
  ...
  private async populatestoryData(stories: StoryModel[], user: UserDto) {
    const performerIds = uniq(stories.map(f => f.fromSourceId.toString()));
    ...
    const [performers, files, actions] = await Promise.all([
      ...
      user && user._id
        ? this.reactionService.findByQuery({
            objectType: REACTION_TYPE.STORY,
    ...
    return stories.map(f => {
      ...
      if (performer) {
        story.performer = performer.toPublicDetailsResponse();
      }
      ...
      const storyFileStringIds = (f.fileIds || []).map(fileId =>
        fileId.toString()
      );
      const storyFiles = files.filter(file =>
        storyFileStringIds.includes(file._id.toString())
      );
      if (storyFiles.length) {
        story.files = storyFiles.map(file => {
          ...
          return {
            ...file.toResponse(),
            thumbnails: (file.thumbnails || []).map(thumb =>
              FileDto.getPublicUrl(thumb.path)
            ),
            url: fileUrl
          };
        });
  ...
  public async createForPerformer(
  ...
  public async updateStory(
    ...
    if (!story || story.fromSourceId.toString() !== user._id.toString())
      throw new EntityNotFoundException();
